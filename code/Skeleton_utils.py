...
# Invoking function
def process_single_pcd(pcd_path, result_folder, radius):
    print(pcd_path)
    ...
    trunk_folder = os.path.join(skeleton_folder, "trunk", os.path.basename(pcd_path)[:-4])
    branch_folder = os.path.join(skeleton_folder, "branch", os.path.basename(pcd_path)[:-4])
    create_folder_if_not_exists(trunk_folder)
    create_folder_if_not_exists(branch_folder)

    trunk_path = os.path.join(trunk_folder, "trunk_" + os.path.basename(pcd_path))
    branch_path = os.path.join(branch_folder, "branch_" + os.path.basename(pcd_path))
    ...
    mesh_folder = os.path.join(skeleton_folder, "mesh", os.path.basename(pcd_path)[:-4])
    os.makedirs(mesh_folder, exist_ok=True)
    # create_folder_if_not_exists(mesh_folder)
    slbc.save(mesh_folder)


def process_point_clouds(pcd_folder, result_folder, slbc_downsampe):
    pcd_files = [os.path.join(pcd_folder, f) for f in os.listdir(pcd_folder) if f.endswith('.pcd')]
    pool = Pool(processes=cpu_count())  # Number of CPU cores used
    results = []
    for pcd_path in pcd_files:
        result = pool.apply_async(process_single_pcd, args=(pcd_path, result_folder, slbc_downsampe))
