...
def extract_two_main_branches(pcd):
    graph = make_graph(pcd, 0.5)
    ...
    points = []
    outside_points = find_outside_points(np.asarray(pcd.points), 0.05, graph)
    for i in range(len(outside_points)):
...
def combine_intensity_and_graph(pcd_remove, pcd_all):
    if pcd_all != None:
        trunk_branches_point, _, max_path, _, _= extract_two_main_branches(pcd_remove)
...
def get_main_branch(pcd):
    branch_path_points = []
    pcd_all = o3d.geometry.PointCloud()
    pcd_all.points = o3d.utility.Vector3dVector(np.asarray(pcd.points))
    trunk_points,outside_need,_,trunk_branches_path,branch_nodes = extract_two_main_branches(pcd)
    ...
    pcd_tree = o3d.geometry.KDTreeFlann(pcd_trunk)
    graph = make_graph(pcd_remove_trunk,0.2)
    # Cluster the remaining parts of the pear tree point cloud
    clusters = cluster_points(pcd_remove_trunk,0.015)
...
def main(input_las_file,skeleton_file_lidar,index,save_file):
    skeleton_lidar_pcd = o3d.io.read_point_cloud(skeleton_file_lidar)
    pcd_lidar,off = write_to_pcd(input_las_file)
    # Use graph theory and LIDAR intensity values to remove support structures
    pcd_remove, skeleton_lidar_pcd_final = remove_support_structure_graph(skeleton_lidar_pcd)
    pcd_intensity, points_intensity_value = remove_support_structure_intensity(input_las_file, skeleton_lidar_pcd_final, pcd_lidar)
    pcd_final, max_path = combine_intensity_and_graph(pcd_remove, pcd_intensity)
    # Skeleton repair
    pcd_whole = repair_local(pcd_final, pcd_lidar, max_path, 0.05, 4)
    pcd_final_repair = repair_tree_dijkstra(pcd_whole)
    # Save the repaired skeleton point cloud file
    points_final_repair = np.asarray(pcd_final_repair.points)+off

    pcd_final_ply = o3d.geometry.PointCloud()
    pcd_final_ply.points = o3d.utility.Vector3dVector(np.asarray(points_final_repair))
    o3d.io.write_point_cloud(os.path.join(save_file, f"{index}.ply"), pcd_final_ply)

    # Compute branching
    branch_nodes,branch_number,tree_points,outside_point = get_main_branch(pcd_final_repair)
    return branch_nodes,branch_number,tree_points,outside_point


def process_point_clouds(las_files_dir, skeleton_files_dir,save_file):
    skeleton_files = [os.path.join(skeleton_files_dir, f) for f in os.listdir(skeleton_files_dir)]
    las_files = [os.path.join(las_files_dir, f) for f in os.listdir(las_files_dir)]
    pool = Pool(processes=cpu_count())
    results = []
    for i in range(len(skeleton_files)):
        result = pool.apply_async(main, args=(las_files[i], skeleton_files[i], i,save_file))
...
if __name__ == "__main__":
    skeleton_files_dir = r"E:\process_skeleton_file"
    las_files_dir = r"E:\process_las_file"
    save_file = r"E:\data_results"
    # Use multiprocessing to handle files
    process_point_clouds(las_files_dir,skeleton_files_dir,save_file)